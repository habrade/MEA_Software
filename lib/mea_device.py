...
class MeaDevice:
    def __init__(self, hw):
        self.hw = hw
        self.mea_base = "mea_dev."
        self.meaclk_base = self.mea_base + "mea_clk_drp."
    ...
    def is_locked(self):
        reg_name = self.mea_base + "mea_clk_locked"
        node = self.hw.getNode(reg_name)
        locked_raw = node.read()
        self.hw.dispatch()
        locked = locked_raw.value()
        log.debug("locked status : {:d}".format(locked))
        if locked == 1:
            return True
        else:
            return False
    ...
    def set_frq(self, frq, prec=0.1):
        mmcm_inst = Mmcm(frq)
        while mmcm_inst.M > mmcm_inst.Mmin:
            ## Set Mea Clocks, parameters: Do, M, D
            ## Frq: (clkin * M)/(DO * D)
            DO, M, D = mmcm_inst.get_parameters()
            freq_calc = mmcm_inst.Fin * M / (DO * D)
            log.debug("Calculating clkout frequency is: {}".format(freq_calc))
            ...
            if diff > prec:
                ...
                log.info("Calculated clkout frequency is: {}".format( freq_calc))
                log.info("Freq diff percent: {:}%".format(diff))
                self.set_clock(DO, M, D)
                if self.is_locked():
                    log.info("Clock is locked!")
                    break
                self.rst_mmcm(True, True)
                log.error("Clock is not locked!")
